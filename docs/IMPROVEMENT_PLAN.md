...
27. ~~Lock-файл для COURSE_STATE.md.~~ **Отклонено:** курс почти всегда ведёт один человек, поэтому lock не даёт пользы, но добавляет риск зависших блокировок.
...
запись Step 3 в `<LAB_DIR>history.md`, классифицирует каждую проблему к её корневому
шагу (plan / notebook / spec / datasets / tests) и называет команду для запуска
(раньше по пайплайну — первой). Ничего не редактирует, состояние не меняет.
Reference: `references/lab_triage.md`.
...
### Шаг 6.1. `scripts/validate_state.py` ✅ реализовано
Проверка COURSE_STATE.md vs реальных файлов: статусы соответствуют наличию артефактов.
Реализовано как `skill/scripts/validate_state.py` (покрыто `tests/unit/test_validate_state.py`);
`/course-maker doctor` использует его как facts-слой.

### Шаг 6.2. `/course-maker doctor` ✅ реализовано
Расширенная проверка: TODO в `course_plan.md`, drift между state и реальностью, отсутствующие зависимости.
Reference: `references/doctor.md`.

### Шаг 6.3. Compaction для `history.md` — отложено; вместо команды — предупреждение в `doctor`
Полноценная команда `/course-maker compact-history N` **отложена** (решение 2026-07-20):
на реальных размерах `history.md` (per-unit, короткие записи ~60–80 строк на активно
итерируемый юнит) пейн пока гипотетический, а лоссиха-компактизация рискует выкинуть
именно отвергнутые идеи — то, ради чего файл и существует (анти-повтор-память).

Вместо этого добавлен лёгкий безрисковый заслон: `validate_state.py` выдаёт
advisory-находку `BULKY`, если любой `history.md` вырос выше `HISTORY_WARN_LINES`
(200 строк); `doctor` показывает её как опциональную «уборку» (не drift, exit-code
не меняет). Полноценную rejection-preserving компактизацию делать только когда
появится реально раздутая многосеместровая история.

### Шаг 6.4. Sanity check перед `slides` ✅ реализовано
В `step4_slides.md` (коммит `36d2510`): шаг сбора контекста делает `ls -l` PNG и
`figures.py`; если любой PNG старше `figures.py` — предупреждение + предложение
перезапустить `/course-maker figures N`. Warning, не hard-block.

### Шаг 6.5. `/course-maker stats` ✅ реализовано
Прогресс-bar по курсу: сколько лекций готово, сколько часов покрыто.
Reference: `references/stats.md`.

### Шаг 6.6. Lock-файл — ОТКЛОНЕНО
~~`COURSE_STATE.md.lock` создаётся в начале команды, удаляется в конце. Защита от параллельных запусков.~~
**Решение:** не делать. Разработкой курса в подавляющем большинстве случаев занимается
один человек, поэтому защита от параллельных запусков не нужна, а lock-файл создаёт
реальный риск зависших блокировок (например, если команда упадёт до удаления `.lock`).
...
2. SKILL.md занимает < 420 строк (цель поднята: 300 → 350 → 420 по мере роста
   числа пайплайнов. Надёжность соблюдения правил важнее длины — inline-CRITICAL
   блоки НЕ выносятся и НЕ сворачиваются ради экономии строк; сжимать можно
   только пустые диспетчеры-указатели в таблицу command→reference. См.
   feedback_skill_md_critical_colocation в памяти проекта).
