...
Plan a test/quiz/exam before generating questions. Output is
`tests/NN/test_plan.md`, instructor-only. Like a lecture or lab plan: interactive,
iterate until approved, log decisions to `tests/NN/history.md`.

## Context to read first

1. `tests/NN/history.md` — past iterations for this test (read before anything).
2. `course_plan.md` — the test's row in the `## Sessions` table (Type `Test`) and
   any topic/weight notes; the `## Overview` for context.
3. `COURSE_STATE.md` — which lectures/seminars are already taught (so the test
...
4. `CLAUDE.md` → `## Course context` and `course_conventions.md` — language and
   terminology. The plan file is English (skill machinery); the test *content*
...
3. **Per block — question types and counts.** The instructor defines the types
   ...
   and the point weight per question. Record a **type legend** so `test generate`
